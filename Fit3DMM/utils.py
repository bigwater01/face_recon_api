...
def project_back_bfm(model, features):
    ...
    T = model.texMU + T
    # Some filtering
    T = [truncate_uint8(value) for value in T]
...
def crop_img(img, tlx, tly, brx, bry, img2, rescale):
    l = float(tlx)
    t = float(tly)
    ww = float(brx - l)
    hh = float(bry - t)
...
def crop_by_input_lm(img, lms, img2):
    nLM = lms.shape[0]
    lms_x = [lms[i, 0] for i in range(0, nLM)]
    lms_y = [lms[i, 1] for i in range(0, nLM)]
    return crop_img(img, min(lms_x), min(lms_y), max(lms_x), max(lms_y), img2, rescaleCASIA)


def crop_by_face_det(img, det, img2):
    return crop_img(img, det.left(), det.top(), det.right(), det.bottom(), img2, rescaleBB)


def crop_by_lm(img, shape, img2):
    nLM = shape.num_parts
